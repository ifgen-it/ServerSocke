...
namespace ServerSocket
{
    class Program
    {
        ...
        private static void InitServer()
        {
            ...
            try
            {
                StreamReader rd = new StreamReader("server_config.txt");
                //string strServer = rd.ReadLine().Trim();
                string strPort = rd.ReadLine().Trim();
                string strTimeout = rd.ReadLine().Trim();
                rd.Close();

                //string[] strServers = strServer.Split(' ');
                string[] strPorts = strPort.Split(' ');
                string[] strTimeouts = strTimeout.Split(' ');
                port = int.Parse(strPorts[1]);
                socketTimeoutInMinutes = int.Parse(strTimeouts[1]);
                //localHost = strServers[1];
                Console.WriteLine("Server configuration loaded from file 'server_config.txt'\n");
                //Console.WriteLine("Server : " + localHost + " : " + port + "\n");
                Console.WriteLine("Server is listening now all Network interfaces on this PC on port : " + port + "\n");
                Console.WriteLine("Timeout for client inactive, after this client will be disconnected : " + socketTimeoutInMinutes + " min\n");
            ...
            {
                Console.WriteLine(ex.Message);
                Console.WriteLine("\nCheck existance of original file 'server_config.txt' in the directory with program.\n" +
                    "In this file you can change Server port and Timeout disconnection for Clients\n");

                Console.WriteLine("Will be used default server options:\n");
                Console.WriteLine("Server port : " + port + "\n");
                Console.WriteLine("Clients timeout : " + socketTimeoutInMinutes + " min\n");
